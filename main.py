...
# Функция проверки устойчивости по последним 40% отклика
def control(ys):
    ref = ys[-1]
    for i in range(int(len(ys)*0.4)):
            if ref != ys[-(i+1)]:
...
def response_plot(deltas, ys, k1, k3, name, mode):
    stability = control(y)
    pl.plot(deltas, ys)
    ...
    pl.ylabel('Выход')
    pl.grid()
    pl.savefig(name, format='png', dpi=1000)
    if mode == 'show':
        pl.show()
    pl.clf()
...
for k1 in k1_list:
    for k3 in k3_list:
        y = out_signal(x, k1, k2, k3)
        filename = 'Output:k1={}k3={}.txt'.format(k1, k3)
        plotname = 'Plot:k1={}k3={}.png'.format(k1, k3)
        record_in_file(filename, x_time, x, y)
        response_plot(x_time, y, k1, k3, plotname, mode)
